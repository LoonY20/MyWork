...
describe("getSize", function(){
    it("size", function(){
        let myArray0=new Alist([1,2,3])
        
        assert.equal(myArray0.getSize(),3);
    });
});
describe("addStart", function(){
    it("start=50", function(){
        let myArray1=new Alist([1,2,3]);
        myArray1.addStart(50);
        assert.equal(myArray1.getSize(),4);
    });
});
describe("addEnd", function(){
    it("end=50", function(){
        let myArray2=new Alist([1,2,3]);
        myArray2.addEnd(50)
        assert.equal(myArray2.getSize(),4);
        assert.equal(myArray2.getIndex(4),50);
    });
});
describe("myToString", function(){
    it("myTostring", function(){
        let myArray3=new Alist([50,100,500,1000])
        assert.equal(myArray3.myToString(),"501005001000");
    });
});
describe("delEnd", function(){
    it("delend", function(){
        let myArray4=new Alist([1,2,3,50]);

        assert.equal(myArray4.delEnd(),50);
        assert.equal(myArray4.getSize(),3);
    });
});
describe("delStart", function(){
    it("start", function(){
        let myArray5=new Alist([1,2,3,4,5]);
        assert.equal(myArray5.delStart(),1);
        assert.equal(myArray5.getSize(),4);
    });
});
describe("Clear", function(){
    it("clear", function(){
        let myArray6=new Alist([1,2,3,4,5]);
        myArray6.addEnd(50);
        myArray6.Clear();
        assert.equal(myArray6.getSize(),5);
        assert.equal(myArray6.Clear(),"1,2,3,4,5");
    });
});
